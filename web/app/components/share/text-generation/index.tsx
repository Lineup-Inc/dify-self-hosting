...
const TextGeneration: FC<IMainProps> = ({
  ...
  const { isPending: isGettingAccessMode, data: appAccessMode } =
    useGetAppAccessMode({
      appId,
      isInstalledApp,
      enabled: systemFeatures.webapp_auth.enabled,
    });
  const { isPending: isCheckingPermission, data: userCanAccessResult } =
    useGetUserCanAccessApp({
      appId,
      isInstalledApp,
      enabled: systemFeatures.webapp_auth.enabled,
      ...
      if (varLen > 0) {
        item.slice(0, varLen).forEach((input, index) => {
          ...
          if (!input) {
            ...
            else inputs[varSchema?.key as string] = undefined;
          }
        });
      }
      return {
  ...
  if (
    ...
    (systemFeatures.webapp_auth.enabled &&
      (isGettingAccessMode || isCheckingPermission))
  ) {
    return (
